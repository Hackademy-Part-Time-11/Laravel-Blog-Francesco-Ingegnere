<div class="bg-black">
    <div class="container py-4">
        <div class="row">
            <div class="col">
                <h6 class="py-2">Scrivi e pubblica il tuo articolo BESTIALE su BEASTS-BLOG</h6>
            </div>
            <div class="col text-end">
                <button type="button" class="btn btn-primary px-5" style="--bs-btn-padding-y: .25rem; --bs-btn-padding-x: .5rem; --bs-btn-font-size: .75rem;"><i class="fa-solid fa-square-phone"></i> Contattaci</button>
            </div>
        </div>
        
        
    </div>
</div>
<div class="bg-body-tertiary">
    <div class="container py-2">
        <div class="row border-bottom mx-auto">
            <div class="row justify-content-center py-2">
                <div class="col-2 text-center text-white">
                    <i class="fab fa-facebook"></i>
                </div>
                <div class="col-2 text-center text-white">
                    <i class="fab fa-instagram"></i>
                </div>
                <div class="col-2 text-center text-white">
                    <i class="fab fa-twitter"></i>
                </div>
                <div class="col-2 text-center text-white">
                    <i class="fab fa-telegram"></i>
                </div>
            </div>
        </div>
        <p class="py-5 mb-0">© Copyright 2012 - 2023 | Beasts Blog by <a href="#">Francesco Ingegnere</a> | Laravel</p>
    </div>
</div>